...
#endif
typedef void (*inic_ipc_promisc_callback_t)(unsigned char *buf, unsigned int len, void *user_data);
...
int inic_ipc_api_dev_message_send(u32 id, u32 *param_buf, u32 buf_len)
{
	...
	ipc_send_message(IPC_DIR_MSG_TX, IPC_D2H_WIFI_API_TRAN, &g_dev_ipc_api_msg);
#else
...
void inic_ipc_autoreconnect_indicate(rtw_security_t security_type, char *ssid, int ssid_len, char *password, int password_len, int key_id)
{
	u32 param_buf[6];
...
void inic_ipc_api_init_dev(VOID)
{
	...
	if (pdTRUE != xTaskCreate((TaskFunction_t) inic_ipc_api_dev_task, (const char *const)"inic_ipc_api_dev_task", INIC_IPC_API_STACK, NULL, tskIDLE_PRIORITY + CONFIG_INIC_IPC_DEV_API_PRIO, &api_dev_task)) {
...
IPC_TABLE_DATA_SECTION const IPC_INIT_TABLE ipc_api_dev_table[] = {

...
#if defined(CONFIG_BT_AP) && CONFIG_BT_AP
	int *ret = NULL;
	ret = bt_ipc_api_host_message_send(RTK_BT_IPC_POWER_STATE, 3, NULL, 0);	/* Inform NP to enter Power save */
	osif_mem_free(ret);
#endif
}
...
#if defined(CONFIG_BT_AP) && CONFIG_BT_AP
	int *ret = NULL;
	ret = bt_ipc_api_host_message_send(RTK_BT_IPC_POWER_STATE, 1, NULL, 0);	/* Inform NP to init Power save */
	osif_mem_free(ret);
#endif
...
#if defined(CONFIG_BT_AP) && CONFIG_BT_AP
	int *ret = NULL;
	ret = bt_ipc_api_host_message_send(RTK_BT_IPC_POWER_STATE, 2, NULL, 0);	/* Inform NP to deinit Power save */
	osif_mem_free(ret);
#endif
